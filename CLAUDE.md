...
- **直近の更新内容**:
  ...
  - パイプライン振り返りページの想定予算を、案件本体の`expectedBudget`ではなく最新の営業記録の`budget`で表示するように変更
    ...
    - `fetchLatestRecordBudget`（`PipelineForecastPage.js`）で「dateが最も新しく、1円以上のbudgetを持つ営業記録」のbudgetを導出し、読み込み時に`expectedBudget`を上書きする。該当レコードが無い案件は従来どおり案件本体の`expectedBudget`にフォールバック
    - 対象は保有中の案件・今期対象外・成約予定リストの表示と、着地予想・四半期合計進捗の見込み計算のすべて（成約予定リスト・四半期合計は区分を問わないため、案件ごとの`isExistingProject`に対応する側のサブコレクションを見る）
    - DB上の`expectedBudget`フィールド自体は書き換えない（他画面への影響なし。表示時の導出のみ）
